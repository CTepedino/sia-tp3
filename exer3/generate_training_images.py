...
def create_number_image(number, size=(35, 35), base_font_size=30, noise_level=0.1, 
    ...
    max_x_offset = min(
        int(size[0] * position_variation),  # Based on position variation
        (size[0] - text_width) // 2  # Based on text width
    )
    max_y_offset = min(
        int(size[1] * position_variation),  # Based on position variation
        (size[1] - text_height) // 2  # Based on text height
    )
...
def generate_dataset(start_num, end_num, images_per_number, output_dir, is_training=True):
    ...
    noise_level = 0.15 if is_training else 0.05
